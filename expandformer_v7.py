"""
ExpandFormer v6: Flash Learning + Diffusion + Adaptive Growth + Real-time Outputs
==================================================================================
...
✓ LIVE OUTPUTS: See model evolve during training!
...
python expandformer_v6.py --stream     # Streaming mode (more verbose)
"""
...
class FlashDiffusionLearner:
    ...
    def diffusion_pass(self, texts, pass_num, total_passes, focus="contextual",
                       sample_every=50, test_prompts=None):
        """
        One diffusion pass at a specific granularity with LIVE OUTPUTS

        Args:
            pass_num: Current pass (1, 2, 3, ...)
            total_passes: Total number of passes
            focus: "abstract", "contextual", or "episodic"
            sample_every: Generate test outputs every N updates
            test_prompts: List of prompts to test, or None for defaults
        """
        print(f"\n" + "=" * 70)
        ...
        print(f"📊 Showing live outputs every {sample_every} updates\n")
        ...
        for text_idx, text in enumerate(texts):
            ...
            for i in sampled_indices:
                ...
                # ⭐ LIVE OUTPUT SAMPLING
                if num_updates - last_sample_update >= sample_every:
                    last_sample_update = num_updates
                    avg_loss = total_loss / num_updates

                    print(f"\n{'─' * 70}")
                    print(f"📊 Update {num_updates:5d} | Loss: {avg_loss:.4f} | Blocks: {len(self.model.get_all_blocks())}")
                    print(f"{'─' * 70}")

                    # Generate from test prompts
                    for prompt in test_prompts:
                        try:
                            output = self.generate(prompt, max_length=25, temperature=0.7)
    ...
    def train_streaming(self, texts, test_prompts=None, sample_every=20):
        """
        STREAMING MODE: Continuous live outputs while learning
        Perfect for watching the model evolve in real-time!
        """
        print("\n" + "=" * 70)
        print("🌊 STREAMING TRAINING MODE")
        print("=" * 70)
        print(f"📊 Live outputs every {sample_every} updates")
        print("   This is VERY verbose - great for watching learning!\n")

        if test_prompts is None:
            test_prompts = ["Hello", "What", "The", "I think"]

        # Single optimizer for streaming
        self.optimizer = torch.optim.AdamW(
            self.model.parameters(),
            lr=self.base_lr,
            weight_decay=0.01
            ...
            for epoch in range(3):  # Multiple passes
                ...
                for text in texts:
                    ...
                    for i in range(len(token_ids) - 1):
                        ...
                        x = torch.tensor([context[-self.model.context_len:]],
                                       dtype=torch.long, device=self.device)
                        y = torch.tensor([token_ids[i + 1]],
                                       dtype=torch.long, device=self.device)
                        ...
                        with torch.no_grad():
                            emb_weight = self.model.embedding.embedding.weight
                            if emb_weight.grad is not None:
                                for tid in range(min(self.model.vocab_size,
                                                    emb_weight.grad.shape[0])):
                                    lr_mult = self.model.embedding.lr_multipliers[tid]
                                    emb_weight.grad[tid] *= lr_mult
                        ...
                        # ⭐ LIVE OUTPUT EVERY N STEPS
                        if num_updates % sample_every == 0:
                            avg_loss = total_loss / num_updates

                            # Rotating prompt
                            prompt = test_prompts[num_updates % len(test_prompts)]

                            try:
                                output = self.generate(prompt, max_length=20, temperature=0.8)
                                # Truncate for single-line display
                                display = output if len(output) <= 50 else output[:50] + "..."
                                print(f"[{num_updates:6d}] Loss: {avg_loss:.3f} | '{prompt}' → {display}")
                            except:
                                print(f"[{num_updates:6d}] Loss: {avg_loss:.3f} | [gen error]")

                    # Update hierarchy periodically
                    ...
                    if num_updates % 200 == 0:
                        if self.model.check_and_split():
                            print(f"  🌱 SPLIT! Now {len(self.model.get_all_blocks())} blocks")

        except KeyboardInterrupt:
            print("\n\n⏸️  Training paused by user")

        print(f"\n\n✅ Streaming complete: {num_updates} updates")
        return total_loss / num_updates if num_updates > 0 else 0

    def train_flash_diffusion(self, texts, num_passes=3, sample_every=50, test_prompts=None):
        """
        Complete training pipeline with live outputs:
        1. Flash scan (already done)
        2. Multi-pass diffusion with live monitoring
        3. Ready for real-time
        """
        print("\n" + "=" * 70)
        print("PHASE 2: DIFFUSION TRAINING (with live outputs!)")
        print("=" * 70)

        if test_prompts is None:
            test_prompts = ["Hello", "What is", "The", "I think", "You"]
...
def train_pipeline(mode='diffusion'):
    """Complete training pipeline with live outputs"""
    print("=" * 70)
    print("🚀 ExpandFormer v6: Flash + Diffusion + Growth + LIVE OUTPUTS")
    print("=" * 70)
    print()
    ...
    # Test prompts for live monitoring
    test_prompts = ["Hello", "What is", "The", "I think", "You are"]

    # PHASE 2: TRAINING
    if mode == 'stream':
        print("\n🌊 STREAMING MODE: Very verbose, shows every update!")
        learner.train_streaming(training_texts, test_prompts=test_prompts, sample_every=20)
    else:
        print("\n🌊 DIFFUSION MODE: Shows periodic samples during training")
        learner.train_flash_diffusion(
            training_texts,
            num_passes=3,
            sample_every=50,  # Adjust this! Lower = more frequent
            test_prompts=test_prompts
        )
    ...
    print("\n" + "=" * 70)
    print("PHASE 3: REAL-TIME INTERACTIVE LEARNING")
    print("=" * 70)
...
def main():
    if len(sys.argv) > 1:
        if sys.argv[1] == '--chat':
            ...
            train_pipeline(mode='stream')
        elif sys.argv[1] == '--help':
            print("ExpandFormer v6: Flash + Diffusion + Growth + Live Outputs")
            print("\nUsage:")
            print("  python expandformer_v6.py              # Train (diffusion mode)")
            print("  python expandformer_v6.py --stream     # Train (streaming mode - very verbose)")
            print("  python expandformer_v6.py --chat       # Chat")
            print("  python expandformer_v6.py --chat NAME  # Chat with specific checkpoint")
            print("\nDiffusion mode: Shows outputs every N updates (configurable)")
            print("Streaming mode: Shows output constantly - watch it learn!")
        else:
            print(f"Unknown: {sys.argv[1]}")
    else:
        train_pipeline(mode='diffusion')
