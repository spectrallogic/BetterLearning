...
class BPETokenizer:
    ...
    def train_bpe(self, texts, verbose=True):
        ...
        if verbose:
            print(f"   Learning {target_merges} merges...")
        ...
        while self.next_id < self.vocab_size:
            ...
            if verbose:
                pbar.update(1)
                if num_merges <= 10:
                    pbar.set_postfix({'example': f"{best_pair[0]}+{best_pair[1]}→{merged}"})
...
def quick_demo():
    """
    Quick demo showing token-based learning with training_data folder
    """
    print("=" * 70)
    ...
    # Load training data from training_data/ folder
    ...
    if training_dir.exists() and training_dir.is_dir():
        ...
        # Find all text files in training directory
        for file_path in training_dir.glob("*.txt"):
            training_files.append(str(file_path))
            try:
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    text = f.read()
                    # Split into paragraphs for training
                    paragraphs = [p.strip() for p in text.split('\n\n') if p.strip()]
                    sample_texts.extend(paragraphs)
                print(f"   ✓ Loaded: {file_path.name} ({len(text)} chars, {len(paragraphs)} paragraphs)")
            except Exception as e:
                print(f"   ⚠️  Error loading {file_path}: {e}")

    if not sample_texts:
        print("⚠️  No training files found in training_data/, using fallback sample data...\n")
        sample_texts = [
    ...
    print(f"\n📊 Total training samples: {len(sample_texts)}")
    print()

    # Train tokenizer on the loaded data
    print("Step 1: Training BPE tokenizer on training data...\n")
    ...
    # Train
    print("Step 4: Training on data with progress tracking...\n")

    # Test prompts to track improvement
    test_prompts = [
        "Hello",
        "How are you",
        "Machine learning",
    ...
    for epoch in range(10):
        ...
        # Progress bar for training samples
        with tqdm(total=len(sample_texts), desc=f"Training", unit="sample") as pbar:
        ...
        print(f"   Tokens Learned: {stats['tokens_learned']:,}")
        print(f"   Templates: {stats['templates']}")

        # Generate samples to show improvement
        print(f"\n🎯 Sample Generations (epoch {epoch+1}):")
        for prompt in test_prompts:
            response = learner.generate(prompt, max_length=15, temperature=0.7)
