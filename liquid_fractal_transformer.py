...
class LiquidHashRouter(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Route through liquid experts using hash-based selection (VECTORIZED!).
        ...
        # Compute hash codes for routing (vectorized)
        hash_codes = self.hash_proj(x)  # (B, T, 16)

        # Simple hash-based expert selection (vectorized)
        expert_scores = torch.abs(hash_codes.sum(dim=-1))  # (B, T)
        expert_indices = (expert_scores % self.n_liquid_experts).long()  # (B, T)

        # Create normalized time values (vectorized)
        t_normalized = torch.arange(T, device=device, dtype=torch.float32) / T  # (T,)
        t_normalized = t_normalized.view(1, -1) * 2 * math.pi  # (1, T)

        # Gather expert parameters for all positions (vectorized)
        # expert_freqs: (n_experts, 8), expert_indices: (B, T)
        selected_freqs = self.expert_freqs[expert_indices]  # (B, T, 8)
        selected_amps = self.expert_amplitudes[expert_indices]  # (B, T, 8)

        # Compute frequency modulations (fully vectorized!)
        # Broadcasting: t_normalized (1, T, 1) * selected_freqs (B, T, 8)
        freq_args = t_normalized.unsqueeze(-1) * selected_freqs  # (B, T, 8)
        modulations = torch.sum(selected_amps * torch.sin(freq_args), dim=-1, keepdim=True)  # (B, T, 1)

        # Apply modulation (vectorized)
        output = x * (1 + 0.1 * modulations)  # (B, T, D)
